...
%define DIVISOR 0x1000   ; Ajustá el divisor como desees
...
breakpoint_paginacion:
    ...
    ; Dirección retornada queda en eax

    ; Cargar CR3 con la dirección física del page directory
    mov cr3, eax

    ; Activar el bit PG (bit 31) del CR0 (también debe estar el PE habilitado)
    mov eax, cr0
    or  eax, 0x80000000  ; Bit PG (paging)
    mov cr0, eax
...
breakpoint_IDT:
    ...
    ; Inicializar y habilitar PIC
    call pic_reset
    call pic_enable

     ; Configurar el divisor del PIT
    mov al, 0x36         ; Modo: Square Wave Generator, canal 0, acceso low/high
    out 0x43, al

    mov ax, DIVISOR
    out 0x40, al          ; Byte bajo
    rol ax, 8
 ...
 ; Cargar el Task Register con la TSS de la tarea inicial
    mov ax, GDT_TASK_INITIAL_SEL   ; Selector completo de la TSS inicial
    ltr ax
